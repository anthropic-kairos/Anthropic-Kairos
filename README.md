...
So this project, Anthropic Kairos, is here as well.

> Anthropic's Kairos to control  
> or  
> Anthropic Kairos to stop

This project aims to connect those scattered but chilling facts into a coherent picture: to warn Claude Code users, and to show through worst-case analysis why open source matters and why public participation in governance matters. This project exists out of a sense of responsibility to humanity. For that reason, I felt I could not remain inactive. Time is short, human resources are limited, and AI assistance was also used in the process, so imperfections are unavoidable. Readers should approach it critically. The better approach is to treat the materials collected here as clues, then investigate and think for themselves.

---

Now consider this question carefully: if an AI infrastructure that is closed-source, deeply pervasive, highly privileged, and strongly oriented toward platform control keeps expanding, while ordinary users remain unable to fully understand its internal mechanisms, the boundaries of its surveillance, the logic of its policy enforcement, or the forces shaping its governance, then what might happen in a bad scenario?

### Your data will not only be mined and viewed, but also used to uniquely identify you

Start with the least severe question: when using Anthropic's model services, is it possible for your personal information to be extracted from different streams of data whenever there is a specific purpose for doing so? Unfortunately, the facts tell us that [Anthropic actively mines conversation data and monitors the data of users it finds interesting.](./src/en/facts/01-Anthropic-is-watching-you.md)
...
If Amodei is willing to proactively mine user data for the sake of corporate interests, could there be another PRISM-style event in the future? But wait — wasn't Amodei widely praised for refusing the U.S. Department of War's demand for mass surveillance? Unfortunately, if you are not part of the 4% of the world's population that is American, but instead part of the other 96%, then Amodei does not care. In [`Statement from Dario Amodei on our discussions with the Department of War`](./references/02-statement-from-dario-amodei-on-our-discussions-with-the-department-of-war.md), he explicitly wrote: "We support the use of AI for lawful foreign intelligence and counterintelligence missions."

So one day, your data — [which may include simple IP addresses, browser user agents, precise device geolocation, hardware fingerprints, keystroke rhythms and interaction-latency patterns, as well as local file structures, running processes, and network-topology metadata](./src/en/facts/03-Anthropic-privacy-and-data-practices-investigation.md) — may end up somewhere inside the Pentagon in the name of "America First."

After learning all this, do you still trust the privacy policy Amodei once drafted? Do you still remember how that privacy policy was changed?

### If it serves its interests, Anthropic will deliberately alter model outputs

[When Anthropic's systems detect that the behavioral pattern of an API account matches signs of distillation, they inject subtle logical errors, misleading code snippets, or invisible special characters into the output](./src/en/facts/03-Anthropic-privacy-and-data-practices-investigation.md), with the goal of interfering with competitors trying to distill its capabilities through the API.
...
3. For example, it could use geographic, biological, and medical information to secretly assassinate high-value targets. It should be remembered that Claude, developed by Anthropic under Amodei's leadership, reportedly played a major role in the operation to capture Venezuelan President Maduro, participated in the war against Iran, and may even have been deeply involved in the attempted assassination of Iran's Supreme Leader Ali Khamenei.
4. For example, it could define attack behaviors to be executed at arbitrary times based on group characteristics. If Kairos were truly implemented, Anthropic's product would amount to a virus hidden inside a computer, waiting to be awakened at any time.
...
Maybe there is — but only if we do not keep clinging to wishful thinking, and only if we stop handing over more permissions, data, workflows, and judgment to a company that is closed-source, unauditable, willing to alter outputs according to its own interests, and openly supports the use of AI for foreign intelligence and counterintelligence missions.
